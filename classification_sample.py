...
parser.add_argument('--nepoch', type=int, default=200, help='Number of epochs to train.')
parser.add_argument('--lr', type=float, default=0.5, help='learning rate.')
parser.add_argument('--wdecay', type=float, default=0.001, help='weight decay (L2 loss on parameters).')
parser.add_argument('--nhop', type=int, default=15, help='Number of hops.')
parser.add_argument('--sample', type=int, default=10, help='Number of negtive samples per node.')
parser.add_argument('--sample_flag', type=bool, default=True, help='Set true if sampling subgraph per epoch.')
...
threshold = 1e-5
model = gutil.ComputeProximity4SVD(ngraph=args.sample_size, niter=args.nhop, dist=args.dist, param=args.param, nclass=num_label).to(device)
...
for epoch in range(args.nepoch):
    ...
    prob = gutil.get_trans_prob_mat(adj, bfs_sample_node_list, args.sample_size, task).to(device)
    Lap_neg = gutil.get_sample_neg_laplacian(args.sample, len(bfs_sample_node_list)).to(device)
    Label_list, node_Label = gutil.get_sample_label_list(Node_Label, bfs_sample_node_list, num_label)
