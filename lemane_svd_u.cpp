...
void Gpush(int* random_w, int start, int end, UGraph* g, vector<Triplet<double>>* answer, double delta){
    ...
    for (int it = start; it < end; it++) {
        ...
        while (!Q.empty()) {
            ...
            mapSPPR[u] += reserve;
            residue -= reserve;
            double update = residue / (double)g->degree[u];
            mapR.erase(uk);
            for(int j = 0; j < g->degree[u]; j++){
                int v = g->AdjList[u][j];
                double threshold = g->degree[v] * delta;
...
int main(int argc,  char **argv){
    ...
    double sppr_norm = sppr_matrix.squaredNorm();
